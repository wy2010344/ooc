import type { DemoEntry } from './types.js'

// 标准库模块（base 包源码，单一来源）：#import 'loop' 复用。
// 浏览器没有文件系统，用笔记承载标准库模块。这里内联一份 base/loop.ooc 内容；
// engine.test 有断言保证本常量与 packages/base/src/loop.ooc 一致，防漂移。
export const LOOP_LIB_SOURCE = `// loop.ooc —— 循环工具（OOC 语言实现，无需宿主桥接）
// 使用：#import 'loop' 后 \`loop apply fn\` / \`loop repeat n fn\`。
// OOC 无控制流关键字：apply 靠方法递归 + #guard 短路（fn 返回真继续，
// 假/NIL/0 时首个重载 guard 不通过、落到同层第二个重载返回 nil 终止）。
// repeat 次数已知，用 JS 生态数据化（n 长度字符串 split 成数组逐索引调用 fn）。
// 注意：repeat 借 'x' repeat n（JS String#repeat）——n 非法时异常由 JS 抛
// （负数 RangeError、小数自动截断），不再像宿主版严格校验非负整数。

loop = {
    // loop apply fn：至少调用 fn 一次，只要它返回真值就继续
    apply(fn) {
        #guard fn apply;
        this apply fn
    },
    // 终止兜底：fn 返回假值时不再继续，返回 nil 静默结束
    apply(fn) => nil,
    // loop repeat n fn：恰好调用 fn n 次（第 i 次传索引 i，从 0 起）
    repeat(n, fn) {
        (('x' repeat n) split '') forEach [v, i => fn apply i];
        nil
    }
};

loop
`

export const loopLib: DemoEntry = {
  name: 'loop.ooc',
  source: LOOP_LIB_SOURCE,
}